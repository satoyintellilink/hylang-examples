...
; decrypt with fernet
; text must be string.
(defn fernet_decrypt [fernet_obj encrypted_text]
  (ebranch (isinstance encrypted_text it)
    (of str)
      (.decode (fernet_obj.decrypt (.encode encrypted_text)))
  )
)
  ...
  (let
    ...
    (let
      ...
      (as-> (read_json (. args infile)) output
        (map_value_deep ((curry fernet_decrypt) fernet_obj) output)
